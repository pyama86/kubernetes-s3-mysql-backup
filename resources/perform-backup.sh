...
do
    ...
    if sqloutput=$(mysqldump -u $TARGET_DATABASE_USER -h $TARGET_DATABASE_HOST -p$TARGET_DATABASE_PASSWORD -P $TARGET_DATABASE_PORT $CURRENT_DATABASE --single-transaction --skip-extended-insert --triggers --routines --events | gzip 2>&1 > /tmp/$CURRENT_DATABASE.sql.gz)
    then
        ...
        if awsoutput=$(aws s3 cp /tmp/$CURRENT_DATABASE.sql.gz s3://$AWS_BUCKET_NAME$AWS_BUCKET_BACKUP_PATH/$CURRENT_DATABASE.sql.gz 2>&1)
        then
